...
if (tgz) {
  ...
  const files = ["dist/index.js", "package.json", "LICENSE", "README.md"];
  ok("tarball contains exactly the 4 expected files", files.every(f => existsSync(join(pkgDir, f))));
  ok("no source shipped", !existsSync(join(pkgDir, "src")));
  ok("no npm token shipped", !existsSync(join(pkgDir, ".npmrc")));
  ok("no MCP server shipped (Claude Code only, as of " + WANT + ")", !existsSync(join(pkgDir, "dist", "mcp.js")));
